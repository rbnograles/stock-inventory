...
export const FinanceSummaryCard = ({
  ...
  return (
    <section
      ...
      className={`relative overflow-hidden rounded-3xl bg-gradient-to-br ${gradient} p-5 text-white shadow-soft`}
    >
      ...
      <div className="relative flex items-center justify-between gap-3">
        ...
        <button
          ...
          className="text-sm font-bold uppercase tracking-[0.16em] text-white/90 disabled:cursor-default"
        >
          {monthLabel}
        </button>
        <button
      ...
      <div className="relative mt-3 flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="truncate text-3xl font-extrabold leading-tight">
            {positive ? "" : "−"}
      ...
      <div className="relative mt-4 grid grid-cols-2 gap-2 text-sm font-semibold">
        <div className="flex items-center gap-2 rounded-2xl bg-white/15 px-3 py-2 backdrop-blur">
          <TrendingUp className="h-4 w-4 flex-none" aria-hidden="true" />
          <div className="min-w-0">
            <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-white/85">Income</p>
            <p className="truncate text-base font-extrabold">{formatMoney(income)}</p>
          </div>
        </div>
        <div className="flex items-center gap-2 rounded-2xl bg-white/15 px-3 py-2 backdrop-blur">
          <TrendingDown className="h-4 w-4 flex-none" aria-hidden="true" />
          <div className="min-w-0">
            <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-white/85">Expense</p>
            <p className="truncate text-base font-extrabold">{formatMoney(expense)}</p>
          </div>
        </div>
      </div>
    </section>
